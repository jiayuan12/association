...
public class UserController {
    ...
    public ResponseResult listByPage(@RequestParam(defaultValue = "1") int pageNum,
                                     @RequestParam(defaultValue = "8") int pageSize) {
        PageInfo<User> userList = userService.getUserPage(pageNum, pageSize, new User());
    ...
    public ResponseResult search(@RequestParam(defaultValue = "1") int pageNum,
                                 @RequestParam(defaultValue = "8") int pageSize,
                                 User search) {
        PageInfo<User> userList = userService.searchUsers(pageNum, pageSize, search);
        return ResponseResultUtil.renderSuccess(userList);
    }
    ...
    @ApiOperation("统计各年级用户数量")
    public ResponseResult getGradeStatistics() {
        List<Map<String, Object>> statistics = userService.getGradeStatistics();
        return ResponseResultUtil.renderSuccess(statistics);
    }

    @GetMapping("/statistics/class")
    @ApiOperation("统计各班级用户数量")
    public ResponseResult getClassStatistics() {
        List<Map<String, Object>> statistics = userService.getClassStatistics();
        return ResponseResultUtil.renderSuccess(statistics);
    }
